<script lang="ts">
  ...
  const rows = [
    { group: 'parse', input: 'small, 177 B', time: '2.81 µs', interval: '2.79 – 2.82 µs', thrpt: '60.1 MiB/s' },
    { group: 'parse', input: 'large, 29,104 B', time: '373 µs', interval: '371 – 376 µs', thrpt: '74.3 MiB/s' },
    { group: 'get', input: 'small, PID-5.1', time: '79.2 ns', interval: '78.4 – 80.1 ns', thrpt: '—' },
    { group: 'get', input: 'large, OBX[200]-5', time: '1.80 µs', interval: '1.77 – 1.83 µs', thrpt: '—' },
    { group: 'tree', input: 'small', time: '13.0 µs', interval: '12.8 – 13.1 µs', thrpt: '—' },
    { group: 'tree', input: 'large', time: '1.44 ms', interval: '1.44 – 1.45 ms', thrpt: '—' },
    { group: 'validate', input: 'small', time: '6.40 µs', interval: '6.38 – 6.42 µs', thrpt: '—' },
    { group: 'validate', input: 'large', time: '602 µs', interval: '598 – 607 µs', thrpt: '—' },
    { group: 'render', input: 'small, 177 B', time: '367 ns', interval: '365 – 369 ns', thrpt: '460 MiB/s' },
    { group: 'render', input: 'large, 29,104 B', time: '28.5 µs', interval: '28.3 – 28.7 µs', thrpt: '974 MiB/s' }
  ];
...
>
  ...
  <p>
    A small message parses in about 2.8 µs, so one core parses on the order of 350,000 ADTs a
    second. For essentially every real HL7 interface, the network, the database, and the downstream
    system decide the throughput. If you are choosing a library on parse speed, you are optimising
    the wrong number.
  </p>
  <h3>Rendering is seven to eight times cheaper than parsing</h3>
  <p>
    About 0.37 µs against 2.81 µs on the same message. That is what “stored as sent, decoded on
    ...
    real, though its size isn't constant either: a previous measurement saw a 13% swing between two
    runs minutes apart, this one saw about 1% — treat the nanosecond rows as a scale, not a point.)
  </p>
  <h3>Use paths, not the tree — this is the useful one</h3>
  <p>
    Reading two fields from the large message costs about 3.6 µs. Building its whole tree costs
    1.44 ms: <strong>nearly 400 times more</strong>. An integration that wants a handful of fields
